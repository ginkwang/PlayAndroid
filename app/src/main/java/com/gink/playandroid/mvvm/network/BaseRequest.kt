...
sealed class BaseRequest<out T> {
    companion object {
        fun <T> onSuccess(data: T): BaseRequest<T> = Success(data)
        fun <T> onLoading(loadingMessage:String): BaseRequest<T> = Loading(loadingMessage)
        fun <T> onError(error: AppException): BaseRequest<T> = Error(error)
    }
...
fun <T> MutableLiveData<BaseRequest<T>>.paresResult(result: BaseResponse<T>) {
    value = when {
        result.isSucces() -> {
            BaseRequest.onSuccess(result.getResponseData())
        }
        else -> {
            BaseRequest.onError(AppException(result.getResponseCode(), result.getResponseMsg()))
        }
    }
}
...
fun <T> MutableLiveData<BaseRequest<T>>.paresResult(result: T) {
    value = BaseRequest.onSuccess(result)
}
...
fun <T> MutableLiveData<BaseRequest<T>>.paresException(e: Throwable) {
    this.value = BaseRequest.onError(ExceptionHandle.handleException(e))
}